...
> **项目**：mem0ai/mem0 —— 给 AI Agent 用的开源记忆层
> **生成方式**：由 **repo-mastery** skill 从源码精读生成（Markdown + HTML 双格式）
> **生成日期**：2026-08-12 | **覆盖**：7 模块 / 23 知识点
> **完整学习状态**：见 [.learning/](.learning/)（进度、复习队列、章节教材、HTML 课程站点）
> **HTML 版课程**：[course.html](course.html)（或直接打开 `.learning/export/index.html`）
...
```
learnings/mem0/
├── course.md                 # ← 你现在在这里（Markdown 主课程）
├── course.html               # HTML 版课程入口（浏览器打开学习）
└── .learning/                # repo-mastery 完整学习状态
    ├── COVERAGE.md           #   课程主文档（与本文内容一致）
    ├── MASTERY.md            #   掌握度仪表盘
    ├── MISSION.md            #   学习使命与边界
    ├── positioning.md        #   生态定位矩阵（mem0 vs 裸向量库/Zep/Letta/LangMem）
    ├── course-map.json       #   课程地图（模块/知识点定义）
    ├── progress.json         #   学习进度 + spaced review 复习队列
    ├── chapters/             #   各模块完整教材（m01~m07）
    ├── notes/                #   全局架构叙事 overview.md
    └── export/               #   HTML 课程站点（index.html + 资源）
```
...
选它 vs 自研的核心判据：**要不要在写路径做 LLM 蒸馏** + 场景要不要时间推理。详见 [.learning/positioning.md](.learning/positioning.md)。
...
## 模块详解（每模块完整教材见 [.learning/chapters/](.learning/chapters/)）
...
23 个知识点均已初始化复习（concept/design 走 Feynman 复述判定，procedure 走量化闸门）。设计类间隔 14 天，概念/程序类 3 天起。复习队列见 `.learning/progress.json`，掌握度仪表盘见 `.learning/MASTERY.md`。

## 后续学习入口

- 读完整教材：[.learning/chapters/m01.md](.learning/chapters/m01.md) … `m07.md`
- 看架构叙事：[.learning/notes/overview.md](.learning/notes/overview.md)
- 看生态定位：[.learning/positioning.md](.learning/positioning.md)
- 交互式 HTML 课程：[course.html](course.html)
- 交互式复习：`/repo-mastery review`（在 mem0 项目 `.learning/` 下执行）
